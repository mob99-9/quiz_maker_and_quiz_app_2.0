...
class WholeGame():
    def select_answer(self, user_answer):
        global answer
        answer = ""
        answer = user_answer
    ...
    def submit(self):
        ...
        if correct_answer == answer:
            ...
            letter_d.destroy()
            submit_button.destroy()
            restart_button = Button(main_window, text="Restart", command=Game.restart_game)
    ...
    def game(self):
        ...
        letter_a = Button(main_window, text=f"{choice_a}", command=lambda: Game.select_answer("A"),
                        activebackground="black")
        letter_a.place(x=100, y=200)

        letter_b = Button(main_window, text=f"{choice_b}", command=lambda: Game.select_answer("B"),
                        activebackground="black")
        letter_b.place(x=100, y=300)

        letter_c = Button(main_window, text=f"{choice_c}", command=lambda: Game.select_answer("C"),
                        activebackground="black")
        letter_c.place(x=500, y=200)

        letter_d = Button(main_window, text=f"{choice_d}", command=lambda: Game.select_answer("D"),
                        activebackground="black")
        letter_d.place(x=500, y=300)

        submit_button = Button(main_window, text="Submit", command=Game.submit)
        submit_button.place(x=340,y=340)

Game = WholeGame()

start_button = Button(main_window, text="Start", command=Game.game)
